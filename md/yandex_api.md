...
Для взаимодействия с API Yandex cloud используется класс Yandex. Все запросы выполняются с помощью функции `createRequest`. Документацию по использованию Яндекс диска можно изучить на [полигоне](https://yandex.ru/dev/disk/poligon/).
...
1. Для загрузки файлов (`uploadFile`):
    ...
    3. данные для запроса `path` (куда загружать) и `url` (откуда загружать)
2. Для загрузки файлов (`removeFile`):
    1. метод `DELETE`
    2. путь `'/resources'`
    3. данные для запроса `path` (откуда удалять)
3. Для получения всех загруженных файлов (`getUploadedFiles`):
    1. метод `GET`
    2. путь `'/resources/files'`
    3. данные для запроса отсутствуют
4. Для всех запросов:
...
2. Настройте ссылку на полученный путь к изображению.
